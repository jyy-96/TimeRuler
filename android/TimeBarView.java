package com.hdec.hikVideoPlayer;
...
public class TimeBarView extends View {
    ...
    /*刻度颜色*/
    private int mScaleColor = 0xFF979797;
    /*时间文字颜色*/
    private int mTextColor = 0xFFB5B9BE;
    /*中线图*/
    ...
    private void initView() {
		this.setBackgroundColor(0xFF15171A);
        //下面的值是初始化后就能确定的
        mMiddleLineDrawable = ResourcesCompat.getDrawable(getContext().getResources(), getResources().getIdentifier("icon_time_bar_center_line", "drawable", getContext().getPackageName()), getContext().getTheme());
        //小刻度大小，大刻度高度 = 小刻度高度*2
        ...
        for (int i = 0; i < scaleNum; i++) {
            if (mDivisorMode == MODE_HOUR) {
                ...
                if (minuteNum % 6 == 0) { //大刻度
                    ...
                    //画出时间文字
                    canvas.drawText(hourMinute, xPosition + i * mDivisorWidth - timeStrWidth / 2, (mViewHeight + mScaleHeight * 2 + mTextSize) / 2.0f, mTextPaint);
                } else {//小刻度
                    //画上面的小刻度
                ...
                for (int j = 0; j < 10; j++) {
                    ...
                    if (j == 0) {
                        ...
                        float timeStrWidth = mTextPaint.measureText(hourMinute);
                        //画出时间文字
                        canvas.drawText(hourMinute, startX - timeStrWidth / 2, (mViewHeight + mScaleHeight * 2 + mTextSize) / 2.0f, mTextPaint);
                    } else {
                        //画上面的小刻度
    ...
    private void drawVideoArea(Canvas canvas) {
        ...
        for (RecordSegment recordSegment : mFileInfoList) {
            //给画笔设置颜色
            mVideoPaint.setColor(ContextCompat.getColor(getContext(), getResources().getIdentifier("playback_timebar_color", "color", getContext().getPackageName())));
            ...
            mVideoAreaRect.set(leftX, 0, rightX, mViewHeight - mScaleHeight * 2);
